...
// The constructor uses an initialization list to set the initial values of the member variables.
Book::Book(const std::string &title, Author* author, const std::string &genre, int publicationYear)
        : title(title), author(author), genre(genre), publicationYear(publicationYear), status(Status::AVAILABLE) {}

// This is the definition of the destructor of the Book class. 
// `default` means that the compiler will generate a destructor with default behavior.
Book::~Book() = default;

// Definitions of the getter methods. 
// `const` after the function name means that these methods do not modify the object's state.
// Definition of the getter methods
